 ...
 *
 * CJS Patch C) Fix dependency compatibility in packaged Electron runtime:
...
// ── CJS Patches (Patch C): Fix dependency compatibility ──────────────────────
// These patches intercept CJS require() to fix modules that break in the
// packaged Electron runtime due to export mismatches.

const origExtension = Module._extensions['.js'];
let cjsPatchCount = 0;

Module._extensions['.js'] = function patchedJsLoader(mod, filename) {
  const normFilename = filename.split(sep).join('/');

  // C1: proper-lockfile — signal-exit v3/v4 export mismatch
  // proper-lockfile expects `const onExit = require('signal-exit')` to be a function (v3),
  // but signal-exit v4 exports { onExit }. This causes "onExit is not a function".
  if (normFilename.includes('proper-lockfile/lib/lockfile.js')) {
    try {
    ...
    } catch { /* fall through to original loader */ }
  }

  // C2: proxy-agent — https-proxy-agent CJS require incompatible with ESM exports
  // Replace CJS require() with dynamic import() for https-proxy-agent.
  // Use /proxy-agent/ with leading slash to avoid matching pac-proxy-agent or https-proxy-agent.
  if (/\/proxy-agent\/dist\/index\.js$/.test(normFilename)) {
    try {
      let src = readFileSync(filename, 'utf8');
      const find = "https: async () => (await Promise.resolve().then(() => __importStar(require('https-proxy-agent')))).HttpsProxyAgent,";
      if (src.includes(find)) {
        src = src.replace(
          find,
  ...
  // C3: pac-proxy-agent — same CJS/ESM mismatch for https-proxy-agent
  if (normFilename.includes('pac-proxy-agent/dist/index.js')) {
    try {
      let src = readFileSync(filename, 'utf8');
      const find = "const { HttpsProxyAgent } = await Promise.resolve().then(() => __importStar(require('https-proxy-agent')));";
      if (src.includes(find)) {
        src = src.replace(
          find,
  ...
  // C4: @discordjs/node-pre-gyp — make proxy agent optional to prevent startup crash
  if (normFilename.includes('@discordjs/node-pre-gyp/lib/install.js')) {
    try {
      let src = readFileSync(filename, 'utf8');
      const find = "const ProxyAgent = require('https-proxy-agent');";
      if (src.includes(find)) {
        src = src.replace(
          find,
  ...
  // C5: node-edge-tts — make https-proxy-agent optional at top-level require
  if (normFilename.includes('node-edge-tts/dist/edge-tts.js')) {
    try {
      let src = readFileSync(filename, 'utf8');
      const find = 'const https_proxy_agent_1 = require("https-proxy-agent");';
      if (src.includes(find)) {
        src = src.replace(
          find,
  ...
  return origExtension(mod, filename);
};

// Log CJS patches on next tick (after all top-level require() calls complete)
process.nextTick(() => {
  if (cjsPatchCount > 0) {
    console.error(`[openclaw-patches] cjs: ${cjsPatchCount} patch(es) applied`);
  }
});
