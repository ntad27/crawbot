...
const loaderSource = [
  ...
  '  if (dispatchDone.has(base) && relayDone.has(base) && streamDone.has(base)) return nextLoad(url, context);',
  '',
  '  let src;',
  ...
  '  }',
  '',
  '  if (dispatchCount > 0 || relayCount > 0 || screenshotCount > 0 || oauthCount > 0 || streamCount > 0) {',
  '    const parts = [];',
